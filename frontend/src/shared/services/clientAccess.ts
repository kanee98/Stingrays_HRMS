...
export async function fetchClientAccessSnapshot(clientKey = getDefaultClientKey()): Promise<ClientAccessSnapshot> {
  ...
  accessUrl.set('clientKey', clientKey);

  const response = await fetch(`/api/admin/public/client-access?${accessUrl.toString()}`, {
    cache: 'no-store',
    credentials: 'omit',
    headers: {
...
export function useClientAccess(clientKey = getDefaultClientKey(), enabled = true) {
