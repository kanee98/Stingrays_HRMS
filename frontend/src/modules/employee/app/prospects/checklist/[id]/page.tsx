...
export default function ProspectChecklistPage() {
  ...
  const handleSendToOnboarding = async () => {
    ...
    try {
      ...
      router.push(employeePath(`/onboarding?employeeId=${data.employeeId}`));
    } catch (e) {
      ...
      <DashboardLayout>
        <div className="space-y-6">
          <PageHeader
            ...
            actions={<Link href={employeePath('/prospects')} className={secondaryButtonClasses}>Back to prospects</Link>}
            ...
            <>
              ...
              <SectionCard eyebrow="Onboarding Handoff" title="Promote to employee" description="Create the employee record from this prospect and continue the onboarding workflow in the next module step.">
                {prospect.ConvertedToEmployeeId ? (
                  <Link href={employeePath(`/onboarding?employeeId=${prospect.ConvertedToEmployeeId}`)} className={secondaryButtonClasses}>
                    Continue onboarding
                  </Link>
                ) : (
