      ...
      <PageHeader
        ...
        actions={
          <>
            ...
            <Link href={employeePath('/onboarding')} className={primaryButtonClasses}>Start onboarding</Link>
          </>
        }
      />
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.9fr)]">
        <SectionCard eyebrow="Quick Actions" title="Move candidates through onboarding" description="Use the same shared action-card pattern as the other modules to keep navigation predictable.">
          <div className="grid gap-3 md:grid-cols-2">
            <ActionCard title="Start employee onboarding" description="Launch a new onboarding workflow for an incoming hire." href={employeePath('/onboarding')} />
            <ActionCard title="Review prospects" description="Track active candidates and checklist completion." href={employeePath('/prospects')} />
            <ActionCard title="Manage templates" description="Maintain the documents used in onboarding workflows." href={employeePath('/templates')} />
            <ActionCard title="Open checklists" description="Standardize readiness and handoff tasks." href={employeePath('/checklists')} />
          </div>
        </SectionCard>
        ...
        <SectionCard eyebrow="Shared UX" title="One platform language" description="Onboarding now uses the same structural page components as HRMS and payroll.">
          <div className="grid gap-3">
            <ActionCard title="Configure step visibility" description="Control optional workflow stages without leaving the shared settings layout." href={employeePath('/settings/onboarding')} actionLabel="Open settings" />
            <ActionCard title="Manage prospect types" description="Control the prospect catalog and Excel import mappings used by the recruitment pipeline." href={employeePath('/settings/prospect-types')} actionLabel="Open settings" />
            <ActionCard title="Manage departments" description="Keep onboarding department lists aligned with the employee intake flow." href={employeePath('/settings/departments')} actionLabel="Open settings" />
            <ActionCard title="Manage document types" description="Update required upload categories using the same settings page pattern." href={employeePath('/settings/document-types')} actionLabel="Open settings" />
          </div>
        </SectionCard>
      </div>
    </div>
