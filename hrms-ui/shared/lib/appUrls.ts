...
function appUrl(port: number): string {
  if (typeof window === 'undefined') {
    // During SSR for client components there is no `window`, so we avoid
    // silently producing localhost links.
    return '';
  }
...
function getServiceUrl(subdomain: string, port: number, envUrl?: string): string {
  if (typeof window === 'undefined') {
    // Avoid a localhost fallback during SSR; use the baked-in URL if present.
    return envUrl ?? '';
  }
