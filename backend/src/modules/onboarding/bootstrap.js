...
function registerOnboardingRoutes(app) {
  ...
  for (const basePath of ['/api/onboarding', '/api/onboarding/api']) {
    app.use(`${basePath}/employees`, employeesRouter);
    app.use(`${basePath}/prospects`, prospectsRouter);
...
async function ensureProspectsTable() {
  try {
    ...
    console.error('Failed to ensure Prospects table:', error.message);
  }
}
...
async function ensureOnboardingSettingsTable() {
  try {
    ...
    console.error('Failed to ensure OnboardingSettings table:', error.message);
  }
}
...
async function ensureOnboardingDocumentTypesTable() {
  try {
    ...
    console.error('Failed to ensure OnboardingDocumentTypes table:', error.message);
  }
}
...
async function ensureDepartmentsTable() {
  try {
    ...
    console.error('Failed to ensure Departments table:', error.message);
  }
}

async function initializeOnboardingModule() {
  await ensureProspectsTable();
  await ensureOnboardingSettingsTable();
  await ensureOnboardingDocumentTypesTable();
  await ensureProspectTypesTable();
