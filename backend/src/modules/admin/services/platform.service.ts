...
import { ensureSuperAdminSessionStore } from "./session.service";
...
async function syncSeedSuperAdminUser(userId: number) {
  const pool = await poolPromise;
  const result = await pool
    .request()
  ...
  const user = result.recordset[0] as { Id: number; FullName: string | null; PasswordHash: string } | undefined;
  if (!user) {
    throw new Error("Failed to resolve seed super admin user");
  }

  const shouldSyncPassword = !(await doesPasswordMatch(DEFAULT_SUPER_ADMIN_PASSWORD, user.PasswordHash));
  const passwordHash = shouldSyncPassword ? await bcrypt.hash(DEFAULT_SUPER_ADMIN_PASSWORD, 12) : null;

  await pool
    .request()
    .input("id", sql.Int, user.Id)
    .input("fullName", sql.NVarChar, DEFAULT_SUPER_ADMIN_NAME)
    .input("refreshName", sql.Bit, shouldRefreshSuperAdminName(user.FullName))
...
async function ensureSeedSuperAdminUser() {
  ...
  if (legacyUser && canonicalUser) {
    ...
    canonicalUserId = retainedUser.Id;
  } else if (legacyUser) {
    await rewriteSuperAdminAuditEmail(pool, LEGACY_SUPER_ADMIN_EMAIL, DEFAULT_SUPER_ADMIN_EMAIL);
    ...
    canonicalUserId = Number(insertResult.recordset[0]?.Id);
  }

  if (!Number.isInteger(canonicalUserId)) {
    throw new Error("Failed to resolve canonical seed super admin user");
  }

  await syncSeedSuperAdminUser(canonicalUserId);
}
...
export async function ensureSuperAdminPlatform() {
  await ensureSchema();
  await ensureSuperAdminSessionStore();
